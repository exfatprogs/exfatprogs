...
static int exfat_rw_bitmap(struct exfat *exfat, bool read)
{
	...
	if (read) {
		rw_len = exfat_read(exfat->blk_dev->dev_fd, exfat->alloc_bitmap,
				    exfat->disk_bitmap_size, exfat_c2o(exfat, exfat->disk_bitmap_clus));
 ...
 * Returns 0 on success.
 */
static int BFS_write_children(struct exfat_defrag *defrag, struct exfat_inode *inode, uint32_t *index)
{
	int ret = 0;
	uint16_t attr, checksum;
	...
	while (1) {
		...
		case EXFAT_FILE:
			...
			for (i = 2; i < dentry_count; i++) {
				...
				if (dentry->type == EXFAT_VENDOR_ALLOC) {
					/* Set start_clu + contiguous_flag */
					dentry->vendor_alloc_start_clu = cpu_to_le32(set->next_clus[(*index)]);
					if ((*index) <= interrupt_point)
 ...
 * Returns 0 on success.
 */
static int data_swap(struct exfat_defrag *defrag, uint32_t clu_1, uint32_t clu_2, bool clu_1_not_free)
{
	struct exfat *exfat = defrag->exfat;
...
static void clus_insert(struct cluster_info_set *set, uint32_t clu_1_prev, uint32_t clu_1_next, uint32_t clu_2)
{
 ...
 */
static void clus_exchange(struct cluster_info_set *set, uint32_t clu_0, uint32_t clu_1, uint32_t clu_2, uint32_t clu_3)
{
 ...
 * 3. exfat_swap_clus uses clus_connect, clus_exchange, clus_insert for metadata update.
 * Returns 0 on success.
 */
static int exfat_defrag(struct exfat_defrag *defrag)
{
	...
	for (i = n_clus; i < n_clus + n_file; i++) {
		...
		exfat_info("\r\033[Kdefrag.exfat has processed %u / %u tasks", i - n_clus + 1, n_file);
		fflush(stdout);
...
static void exfat_defrag_assess(struct exfat_defrag *defrag)
{
	...
	uint32_t num_alloc_breakpoint = 0, num_free_breakpoint = 0, num_free_clus = 0, num_alloc_clus = 0;
	...
	last_clu = 0;
	for (cur_clu = EXFAT_FIRST_CLUSTER; cur_clu < exfat->clus_count + EXFAT_FIRST_CLUSTER; cur_clu++) {
	...
	exfat_info("1. Allocated space: cluster count = %u (%.2lf%%), breakpoint count = %u, fragmentation rate = %.2lf%%\n",
			num_alloc_clus, alloc_space_rate, num_alloc_breakpoint, alloc_space_frag_rate);
...
int main(int argc, char *argv[])
{
	...
	if (signal(SIGINT, sigint_handler) == SIG_ERR || signal(SIGTERM, sigint_handler) == SIG_ERR) {
